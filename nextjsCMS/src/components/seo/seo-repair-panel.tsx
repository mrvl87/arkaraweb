...
function linesFallback(items: string[]): string {
  return items.map((item) => item.trim()).filter(Boolean).length
    ? items.map((item) => `- ${item.trim()}`).join('\n')
    : '(kosong)'
}

function faqToText(items: Array<{ question: string; answer: string }>): string {
  return items.length
    ? items.map((item) => `Q: ${item.question}\nA: ${item.answer}`).join('\n\n')
    : '(kosong)'
}

function proposedFaqToText(items: GenerateSeoRepairPlanOutput['proposed_faq']): string {
  return items.length
    ? items.map((item) => `Q: ${item.question}\nA: ${item.answer}`).join('\n\n')
    : '(kosong)'
}

function contentPatchAfterText(patch: GenerateSeoRepairPlanOutput['content_patch']): string {
