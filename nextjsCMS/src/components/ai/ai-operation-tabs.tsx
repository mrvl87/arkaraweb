...
const PROFILE_OPTIONS: Array<{
...
}> = [
  {
    value: "workspace",
    label: "Workspace",
    description: "Profil umum, sama seperti behavior lama",
  },
  {
    value: "post",
    label: "Post",
    description: "Arahkan output sebagai blog post editorial",
  },
  {
    value: "panduan",
    label: "Panduan",
    description: "Arahkan output sebagai panduan teknis",
  },
];
          ...
          {PROFILE_OPTIONS.map((profile) => {
            ...
            return (
              <button
                ...
                className={`rounded-xl border px-3 py-2 text-left transition-all disabled:cursor-not-allowed disabled:opacity-60 ${
                  isActive
                    ? "border-arkara-amber bg-arkara-amber/10 text-arkara-green shadow-sm"
                    : "border-gray-100 bg-gray-50 text-gray-500 hover:border-arkara-amber/40 hover:bg-white disabled:hover:border-gray-100 disabled:hover:bg-gray-50"
                }`}
              ...
              <div className="mb-2 flex items-center gap-3">
                <div
                  className={`rounded-xl p-2 transition-colors ${
                    isActive
                      ? "bg-arkara-amber/20"
                      : "bg-gray-50 group-hover:bg-arkara-amber/10"
                  }`}
                >
                  <Icon
                    className={`h-5 w-5 ${isActive ? "text-arkara-amber" : operation.color}`}
                  />
                </div>
                <span
                  className={`text-sm font-bold ${isActive ? "text-arkara-green" : "text-gray-700"}`}
                >
                  {operation.label}
                </span>
              </div>
